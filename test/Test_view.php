...
<?php require("../html/htmlStart.php"); ?>
	<br>
	<form method="GET" action="visualiza_geral.php">
		<label for="qtd_geral"
		...
		<select onchange="this.form.submit()"
			...
			while ($resultado_select = mysql_fetch_assoc($result_select))
			{
				...
				if($_GET['compet'] == $resultado_select['COMPETENCIA']) {echo(' selected');}
	...
	<input type="text"
	       id="qtd_geral"
	       value="<?php echo($v_prescons+$v_suger+$v_safin+$v_sijuc+$v_sunop+$v_suasf+$v_sunat); ?>"
	       size="6"
	       readonly>
	<label for="qtd_geral"
	...
	<input type="text"
	       id="qtd_geral"
	       value="<?php echo($imp_prescons+$imp_suger+$imp_safin+$imp_sijuc+$imp_sunop+$imp_suasf+$imp_sunat); ?>"
	       size="6"
	       readonly>
	<br>
	<br>
